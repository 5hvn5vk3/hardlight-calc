...
2. ボタン押下で EyeDropper 起動（**十字カーソル**）。
3. 取得色を基本色として保持。
4. EyeDropper終了後、`結果色を取得する` ボタンを表示。
5. ボタン押下で再度 EyeDropper 起動（**十字カーソル**）。
...
- 正方向ハードライト関数 `hardLight(base, blend)` を以下で定義:
  - `blend < 128` のとき: `round((2 * base * blend) / 255)`
  - `blend >= 128` のとき: `round(255 - (2 * (255 - base) * (255 - blend)) / 255)`
- 丸め: `Math.round`
- 最終値は `clamp(0,255)`
...
1. `s = 0..255` を全探索
2. `hardLight(base, s) === result` を満たす `s` を候補集合に追加
3. 候補が空なら「解なし」
